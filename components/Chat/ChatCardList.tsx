...
const chatData: React.ComponentProps<typeof ChatCard>[] = [
  {
    profileUrl: 'https://cdn.create.vista.com/api/media/small/138013506/stock-vector-user-profile-group',
    name: 'Brien Myell',
    time: 'Thursday',
...
export default function ChatCardList() {
  return (
    <FlatList data={chatData} renderItem={({item,index}) => (<ChatCard key={index} name={item.name} profileUrl={item.profileUrl} time={item.time} currentMessages={item.currentMessages} />)} />
